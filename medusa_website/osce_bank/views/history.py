...
class CategoryProgressTable(tables.Table):
    ...
    cat_qs_num = tables.Column(verbose_name="Nº osce_stations total", empty_values=(), orderable=False)
    attempted_num = tables.Column(verbose_name="Nº osce_stations attempted", empty_values=(), orderable=False)
    cat_average_score = tables.Column(verbose_name="Attempts correct avg (%)", empty_values=(), orderable=False)

    def render_name(self, value, record):
        if value != "OVERALL":
            cat_url = f'{reverse("osce_bank:osce_station_list")}?category={record["id"]}'
...
class SessionHistoryTable(tables.Table):
    ...
    total_session_qs = tables.Column(verbose_name="Nº osce_stations total", empty_values=(), orderable=False)
    attempted = tables.Column(verbose_name="Nº attempted osce_stations", empty_values=(), orderable=False)
    ...
    def render_id(self, value, record):
        session_url = reverse("osce_bank:quiz_session_detail", kwargs={"id": record["id"]})
        return format_html(f'<a href="{session_url}">Review {value}</a>')


class OSCEHistoryView(LoginRequiredMixin, TemplateView):
    template_name = "osce_bank/history.html"

    @method_decorator(login_required)
    def dispatch(self, request, *args, **kwargs):
        return super(OSCEHistoryView, self).dispatch(request, *args, **kwargs)

    def get_context_data(self, **kwargs):
        context = super(OSCEHistoryView, self).get_context_data(**kwargs)
